...
- [ ] **End-to-End Test Issues**: fluxo completo do usuário — via `pytest` marker `smoke` no backend e cenário de browser (agente `gem-browser-tester`) no frontend
...
### Template de Issue de Teste E2E

> ⛔ Este repo **não tem Playwright**. Backend se testa com `pytest`; frontend, por cenário de
> browser executado pelo agente `gem-browser-tester`. Ver `.claude/instructions/stack.md`.

```markdown
# Testes E2E: {Story/Componente}

## Escopo
{história de usuário ou componente sendo testado}

## Design de caso de teste
**Técnica**: {técnica escolhida}
**Tipo**: {Funcional / Não-funcional / Estrutural / Regressão}

## Casos — backend (`apps/api`, pytest)
- [ ] Caminho feliz — `plane/tests/smoke/` ou `contract/app/`
- [ ] Erro e validação (400 com mensagem específica)
- [ ] Autorização: não-membro (403), GUEST escrevendo (403), membro inativo
- [ ] Isolamento cross-workspace (obrigatório)
- [ ] Valores de borda (vazio, nulo, limite de paginação)

## Casos — frontend (cenário de browser)
- [ ] Fluxo principal: passos + resultado observável na tela
- [ ] Estado vazio e estado de erro
- [ ] Responsivo: layout em viewport móvel
- [ ] Sem erro de console durante o fluxo

## Tarefas
- [ ] Escolher marker (`unit` / `contract` / `smoke`) e diretório corretos
- [ ] Reaproveitar fixtures de `conftest.py` e factories de `factories.py`
- [ ] Escrever a matriz de cobertura no topo do arquivo de teste
- [ ] Registrar os cenários de browser para o Stage UX

## Critérios de aceite
- [ ] `cd apps/api && python -m pytest` verde
- [ ] `pnpm --filter <app> check:types` verde
- [ ] Cenários de browser executados e evidenciados (screenshot/console)
