...
- [x] **Fase 16: Forgot Password para clientes do portal**
  ...
  - [x] `PublicHelpdeskCustomerForgotPasswordEndpoint` (`POST /helpdesk/public/portals/<slug>/auth/forgot-password/`): resposta genérica sempre 200 (não revela se o e-mail existe); dispara `helpdesk_forgot_password.delay(...)` via Celery apenas para clientes ativos (`is_active=True`). Protegido por `HelpdeskPublicAuthThrottle` (10/min).
  - [x] `PublicHelpdeskCustomerResetPasswordEndpoint` (`POST /helpdesk/public/portals/<slug>/auth/reset-password/`): decodifica o JWT, valida `purpose`/`workspace_id`/`pwd_sig`, bloqueia contas desativadas, valida força da senha com `zxcvbn` (mesmo threshold do fluxo principal do Plane, score < 3 rejeitado). Protegido por `HelpdeskPublicAuthThrottle` (10/min).
  - [x] Task `apps/api/plane/bgtasks/helpdesk_forgot_password_task.py` + template `apps/api/templates/emails/helpdesk/forgot_password.html`: resolve e prioriza SMTP customizado do portal antes do fallback global da instância, com `normalize_tls_ssl()` e sanitização de URL. Link aponta para `/helpdesk/p/<slug>/reset-password?token=...`.
  - [x] Frontend: link "Forgot password?" na página de login preservando `?next=`; novas páginas `/helpdesk/p/[publicSlug]/forgot-password` (pede e-mail, normaliza com `.trim().toLowerCase()`, mostra confirmação com ação de novo envio) e `/helpdesk/p/[publicSlug]/reset-password` (lê `token` da query string, toggle de visibilidade `Eye`/`EyeOff`, orientação de força, recuperação de erro contextual, define nova senha, redireciona para login). Rotas registradas em `core.ts` antes do catch-all `:requestId`.
  - [x] `PublicHelpdeskService` ganhou `forgotPasswordCustomer` / `resetPasswordCustomer`; `public-helpdesk.store.ts` expõe as mesmas actions como passthrough (sem estado observável novo). Suíte de testes dedicada em `test_helpdesk_auth_hardening.py`.

- [x] **Fase 17: Anexos (4 superfícies)**
  - [x] **Vínculo sem migration:** `HELPDESK_COMMENT_ATTACHMENT` e `HELPDESK_REQUEST_ATTACHMENT` adicionados a `FileAsset.EntityTypeContext`. O campo `entity_type` é `CharField` **sem `choices=`**, então os valores novos não geram migration; o índice composto `asset_entity_idx` já cobre o lookup. Escolhido sobre FK novo porque os modelos de helpdesk usam soft delete (o `ON DELETE CASCADE` nunca dispararia) e um FK exigiria `ALTER TABLE` na maior tabela do banco.
  - [x] **`S3Storage.read_object`** (`plane/settings/storage.py`): não existia método de leitura — só `upload_file`, `copy_object`, `generate_presigned_*`, `get_object_metadata` e `delete_files`. Necessário para o outbound anexar bytes ao email.
  - [x] **`plane/app/helpdesk/attachments.py`** (novo): `bind_assets()`, `assets_for()` (bulk, anti-N+1), `store_inbound_attachment()`, `synthesize_content()`, `attachments_fingerprint()`. Existe porque há **quatro caminhos de escrita sem código compartilhado** (viewset via serializer, endpoint público via `.objects.create()` cru, submit de form e webhook inbound) — todos chamam o mesmo helper em vez de refatorar o endpoint público.
  - [x] **Endpoints** (`plane/app/views/helpdesk/asset.py`, novo): `HelpdeskAssetEndpoint` (POST/PATCH/DELETE ≥ MEMBER, GET download ≥ GUEST) e `PublicHelpdeskAssetEndpoint` (`AllowAny` + `HelpdeskPublicAssetThrottle` a 30/min). O cliente do portal **não é um `User` do Django**, então nenhum endpoint de `asset/v2.py` servia.
  - [x] **Download público endurecido:** anexo de nota interna nunca é servido (`is_internal=False` obrigatório); asset precisa pertencer ao workspace do portal e estar vinculado; em portal com `require_login`, só o dono do ticket lê. Retorna 404 em vez de 403 — a existência do asset é informação.
  - [x] **Claim de anexo órfão:** upload acontece antes do comentário existir, então o asset nasce com `entity_identifier=NULL` e é reivindicado no `bind_assets`. Filtro de segurança: não vinculado + mesmo workspace + `is_uploaded=True` + janela de 24h + cap de `HELPDESK_MAX_ATTACHMENTS_PER_COMMENT`.
  - [x] **Ordem do bind:** sempre **antes** do `publish()` do SSE e do `send_helpdesk_comment_email.delay()` — senão o agente e o worker leem o comentário sem os arquivos.
  - [x] **Inbound (`inbound.py`):** anexos do SendGrid (`attachment1..N`) eram **silenciosamente descartados** — o `post()` lia só `headers`/`from`/`text`/`html`. Agora consome `request.FILES`, faz upload **depois** da autorização (subir blobs antes tornaria o webhook um vetor de enchimento de storage), e sintetiza `content` quando o email traz só anexo.
  - [x] **`empty_body` reordenado:** a checagem passou a considerar anexos, senão um cliente que responde só com um print perderia a mensagem. `content` continua obrigatório no modelo — o placeholder descreve o que chegou em vez de inventar texto do cliente.
  - [x] **Fingerprint de anexos no Message-ID sintético:** `_build_synthetic_message_id` usava `text_body.strip()` como chave; com texto vazio, dois emails só-anexo do mesmo remetente colidiriam e o segundo viraria duplicata.
  - [x] **Teto de 5 MB resolvido:** `RequestBodySizeLimitMiddleware` força `request.body` em toda requisição, o que estende `DATA_UPLOAD_MAX_MEMORY_SIZE` ao multipart (o Django normalmente o isenta). Solução: `BODY_SIZE_EXEMPT_PATHS` isenta só a rota inbound, restaurando o streaming para disco, + guarda de `CONTENT_LENGTH` contra `HELPDESK_INBOUND_MAX_BODY_SIZE` (30 MB) na view. Retorna **200**, não 413 — um payload grande demais nunca cabe num retry, e 413 só gera loop de re-entrega no SendGrid. Matchers de rota adicionados em `Caddyfile.ce` e `Caddyfile.aio.ce`.
  - [x] **Outbound (`helpdesk_email_task.py`):** `_collect_attachments()` lê os assets e monta as triplas do `msg.attach()`, com **`try/except` por asset** — sem isso uma falha de S3 cairia no `except` genérico da task e marcaria o comentário inteiro como `FAILED`, custando ao cliente até o texto. Cap de `HELPDESK_OUTBOUND_ATTACHMENT_MAX_TOTAL_SIZE` (10 MB) com aviso no corpo quando algo é omitido. `S3Storage()` sem `request` — o worker precisa do endpoint interno do MinIO.
  - [x] **Limpeza de órfãos:** `delete_unuploaded_file_asset` só cobre `is_uploaded=False`. Novo `delete_unbound_helpdesk_assets` (beat 02:15 UTC) coleta assets que subiram mas nunca foram enviados.
  - [x] **Serializers:** `HelpdeskAttachmentSerializer` (não-ModelSerializer — os campos úteis vivem no JSON `attributes`, e a URL de download depende da superfície que pergunta, via `public_slug`/`workspace_slug` no contexto). `attachments` adicionado a `HelpdeskRequestCommentSerializer` e `HelpdeskRequestSerializer`, com bulk-fetch no contexto para evitar N+1.
  - [x] **Frontend compartilhado:** `IHelpdeskAttachment` e `IHelpdeskAssetUploadResponse` em `@plane/types`; métodos de credencial/confirmação/delete nas duas classes de `HelpdeskService`. Hook `useAttachmentUpload` em `core/components/helpdesk/attachments/` parametrizado por um `transport`, para servir agente e portal com o mesmo fluxo de 3 passos. Reusa `getFileMetaDataForUpload` (detecta MIME por assinatura e bloqueia extensões perigosas) e `generateFileUploadPayload` de `@plane/services`.
  - [x] **UI:** `AttachmentPicker` (botão de clipe) na barra de ações do composer do agente e do portal; `PendingAttachmentChips` entre o textarea e a barra; `CommentAttachments` dentro da bolha da mensagem.
  - [x] **`helpdeskService` declarado em `IHelpdeskStore`:** a classe já o expunha mas a interface não, o que deixava 2 erros de tipo pré-existentes na página de settings — corrigidos de passagem.
  - [x] **Testes:** 17 novos (46 → 63). `test_helpdesk_attachments.py` cobre as recusas do `bind_assets` (cross-workspace, já vinculado, não enviado, fora da janela, cap, entrada inválida) e o inbound (email só-anexo, `empty_body` como regressão, `payload_too_large` sem parsear, remetente não autorizado **não** tocando storage, dois só-anexo distintos). E2E com Mailpit: anexo chega **byte a byte** íntegro; nota interna com anexo não sai e nem lê o storage. `mailpit.py` ganhou `attachments()` e `download_attachment()`.
