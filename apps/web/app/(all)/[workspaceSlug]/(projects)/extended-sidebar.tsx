...
/** Pure helper: kept at module scope so it is not rebuilt on every render. */
...
export const ExtendedAppSidebar = observer(function ExtendedAppSidebar() {
  ...
  const sortedNavigationItems = useMemo(() => {
    ...
    return (
      WORKSPACE_SIDEBAR_DYNAMIC_NAVIGATION_ITEMS_LINKS.filter((item) => hasPageAccess(slug, item))
        // Copy-on-write is required: the items are shared module constants, so
        // mutating them in place would corrupt every other consumer.
